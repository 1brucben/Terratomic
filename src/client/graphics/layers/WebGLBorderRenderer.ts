...
export class WebGLBorderRenderer implements BorderRenderer {
  ...
  private buildBorderEdges(
    ...
    const color = this.theme.borderColor(owner);

    // Compute neighbor relationship flags
    ...
    if (myPlayer) {
      ...
      for (const neighbor of neighbors) {
        if (!neighbor || !this.game.hasOwner(neighbor)) continue;
        const neighborOwner = this.game.owner(neighbor) as PlayerView;
        if (neighborOwner.id() === owner.id()) continue;

        if (neighborOwner.isFriendly(myPlayer)) {
          hasFriendly = true;
        }
        if (neighborOwner.hasEmbargoAgainst(myPlayer)) {
          hasEmbargo = true;
        }
      }
    }
