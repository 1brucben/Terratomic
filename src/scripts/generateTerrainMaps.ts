...
async function generateGameMapType(maps: MapConfig[]) {
  const enumContent = maps
    .map((map) => `  ${map.fileName} = "${map.displayName}",`)
    .join("\n");

  const fileContent = `// This file is automatically generated by generateTerrainMaps.ts
...
`;
  ...
  console.log(`Generated GameMapType.ts at ${outputPath}`);
}
...
async function loadTerrainMaps() {
  ...
  await generateGameMapType(maps);
